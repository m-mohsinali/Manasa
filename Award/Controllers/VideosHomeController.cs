...
{
    [Authorize]
    public class VideosHomeController : BaseController
    {
        private readonly AwardDbContext _context;
        private readonly IFileInfo _fileInfo;
        private readonly IWebHostEnvironment _hostEnv;
        private readonly ILdapAuthenticationService _ldapAuthenticationService;
        public VideosHomeController(AwardDbContext context, IFileInfo fileInfo, IWebHostEnvironment hostEnv, ILdapAuthenticationService ldapAuthenticationService) : base(context)
        {
            _context = context;
            _fileInfo = fileInfo;
            _hostEnv = hostEnv;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return View(await _context.VideosHome.ToListAsync());
        }
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var announcement = await _context.VideosHome
        ...
        public async Task<IActionResult> Create(VideosHomeViewModel announcementVm)
        {
            //await getLangStrings(PageType.MainWin).ConfigureAwait(false);
            ...
            announcementVm.VideosHome.ThumbnailImagePath = await _fileInfo
            ...
            announcementVm.VideosHome.Name = System.IO.Path.GetFileName(announcementVm.ThumbnailImage.FileName);
            announcementVm.VideosHome.CreatedAt = DateTime.Now;
            _context.Add(announcementVm.VideosHome);
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var videosHome = await _context.VideosHome.FindAsync(id);
            var announcementVideo = new VideosHomeViewModel();
            announcementVideo.VideosHome = videosHome;
            if (videosHome == null)
            {
                return NotFound();
            }
            return View(announcementVideo);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, VideosHomeViewModel vmHome)
        {
            //await getLangStrings(PageType.MainWin).ConfigureAwait(false);
            if (id != vmHome.VideosHome.Id )
            {
                return NotFound();
            }
            ...
            try
            {
                if (vmHome.ThumbnailImage != null)
                {
                    vmHome.VideosHome.ThumbnailImagePath = await _fileInfo
                   .SaveUploadFile(_hostEnv.WebRootPath,
                       AppSetting.Instance.AnnouncementImagePath,
                                   vmHome.ThumbnailImage).ConfigureAwait(false);
                    vmHome.VideosHome.Name = System.IO.Path.GetFileName(vmHome.ThumbnailImage.FileName);

                }
                _context.Update(vmHome.VideosHome);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!AnnouncementExists(vmHome.VideosHome.Id))
                {
                    return NotFound();
                }
                else
        ...
        {
            //await getLangStrings(PageType.MainWin).ConfigureAwait(false);
            var announcement = await _context.VideosHome.FindAsync(id);
            _context.VideosHome.Remove(announcement);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
        ...
        private bool AnnouncementExists(long id)
        {
            ...
            return _context.VideosHome.Any(e => e.Id == id);
        }
